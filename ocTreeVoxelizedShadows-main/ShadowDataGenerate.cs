...
public class ShadowDataGenerate : MonoBehaviour
{
    ...
    public IEnumerator getShadowData(int xLen, int yLen, int zLen, Vector3 offsetWpos, int unitsPerMeter,
        bool useLightSpace, Action<LinkedList<BOcTree.int3>> onFinish)
    {
        ...
        renderTexture = RenderTexture.GetTemporary(rtSize, rtSize, 24, RenderTextureFormat.ARGBFloat,
        ...
        calShadow.SetFloats("MatrixVP",
            getFloatArrayFromMatrix(GL.GetGPUProjectionMatrix(cmr.projectionMatrix, false) * cmr.worldToCameraMatrix));
        ...
        for (int k = 0; k < callCount; k++)
        {
            ...
            for (int i = 0, len = countData[0]; i < len; i++)
            {
                ...
                Vector3 wpos;
                if (useLightSpace)
                    wpos = cmr.transform.localToWorldMatrix.MultiplyPoint3x4(
                        new Vector3(x - 60 * unitsPerMeter, y - 60 * unitsPerMeter, z) / unitsPerMeter) + offsetWpos;
                else
